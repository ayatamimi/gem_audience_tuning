...
class TransformerConfig:
    # data
    train_latents: str; train_indices: str; val_latents: str; val_indices: str
...
def _interpolate_dict(d: dict) -> dict:
    ...
    def _resolve_ref(key_path: str):
        ...
        # 2️⃣ Try dotted path lookup in dict
        cur = d
    ...
    # 3️⃣ Repeat until all placeholders are resolved (max 5 passes for safety)
    for _ in range(5):
...
def _load_config_file() -> dict:
    ...
    with open(cfg_path, "r") as f:
        ...
        text = os.path.expandvars(f.read())  # expands ENV vars like ${RUN_ID}
        cfg = yaml.safe_load(text) or {}
        cfg = _interpolate_dict(cfg)          # expands ${data.run_id} etc.
        return cfg
...
def get_transformer_config() -> TransformerConfig:
    ...
    return TransformerConfig(
        ...
        embed_dim=int(_get(f, "model.embed_dim", 27)), #embed_dim if embed_dim else 64)),
        vocab_size=int(_get(f, "model.vocab_size", 64)),#512)),
        num_layers=int(_get(f, "model.num_layers", 6)),
        num_heads=int(_get(f, "model.num_heads", 9)),#8)),
        hidden_dim=int(_get(f, "model.hidden_dim", 64)),#512)),
        dropout=float(_get(f, "model.dropout", 0.1)),
        mask_prob=float(_get(f, "model.mask_prob", 0.75)),
...
def get_config() -> Config:
    ...
    codebook_size    = int(os.getenv("CODEBOOK_SIZE", _get(f, "model.codebook_size", 64)))
    ...
    latent_channel   = int(os.getenv("LATENT_CHANNEL", _get(f, "model.latent_channel", 32)))
