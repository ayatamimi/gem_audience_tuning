...
def train(cfg, resume_from=None):
    ...
    # Load latent data
    train_latents = np.load('/local/altamabp/audience_tuning-gem/vqvae/AUD-13/latents/train_latents.npy')#(cfg.train_latents)
    train_indices = np.load('/local/altamabp/audience_tuning-gem/vqvae/AUD-13/latents/train_indices.npy')#(cfg.train_indices)
    train_labels = np.load('/local/altamabp/audience_tuning-gem/vqvae/AUD-13/latents/train_labels.npy')
    
    val_latents = np.load('/local/altamabp/audience_tuning-gem/vqvae/AUD-13/latents/val_latents.npy')#(cfg.val_latents)
    val_indices = np.load('/local/altamabp/audience_tuning-gem/vqvae/AUD-13/latents/val_indices.npy')#(cfg.val_indices)
    val_labels = np.load('/local/altamabp/audience_tuning-gem/vqvae/AUD-13/latents/val_labels.npy')
    ...
    model = MaskedLatentTransformer(
        embed_dim=27,#cfg.embed_dim,
        vocab_size=cfg.vocab_size,
        num_layers=cfg.num_layers,
        num_heads=3,#cfg.num_heads,
    ...
    for epoch in range(cfg.epochs):
        ...
        for zq, idx, labels in tqdm(train_loader, desc=f"Epoch {epoch+1}/{cfg.epochs} [Train]"):
            ...
            masked_exp_mask_feat_train_quantizes = attach_bias_mask_feat(
                labels, zq_masked, mask, cfg.mask_prob
                )
            
            optimizer.zero_grad()
            logits = model(masked_exp_mask_feat_train_quantizes)#(zq_masked)
            if cfg.loss_masked:
        ...
        with torch.no_grad():
            for zq, idx, labels in tqdm(val_loader, desc=f"Epoch {epoch+1}/{cfg.epochs} [Val]"):
                ...
                logits = model(masked_exp_mask_feat_val_quantizes)#(zq_masked)
                if cfg.loss_masked:
                    loss = criterion(logits[mask], target[mask])
                else:
                    logits_flat = logits.view(-1, logits.size(-1))
