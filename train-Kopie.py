...
###############################added
    ...
    pretrained_ckpt = "/local/altamabp/audience_tuning-gem/vqvae/AUD-145/vqvae_val_best.pt"  # or cfg.init_ckpt if you add it to config
    state = torch.load(pretrained_ckpt, map_location=device)
    raw_model.load_state_dict(state, strict=True)
    
    if is_main_process():
        print(f"Loaded pretrained VQ-VAE from {pretrained_ckpt}")
        
        
            # ---- freeze encoder, decoder, and projection conv ----
    for p in raw_model.enc_b.parameters():
        p.requires_grad = False
    
    for p in raw_model.dec.parameters():
        p.requires_grad = False
    
    for p in raw_model.quantize_conv_b.parameters():
        p.requires_grad = True

    
    # ---- optimizer: quantizer-only if it has trainable params; otherwise None (EMA-only) ----
    q_params = [p for p in raw_model.quantize_b.parameters() if p.requires_grad]
    
    if len(q_params) == 0:
        if is_main_process():
            print(
                "[WARN] quantize_b has no trainable params (EMA mode). "
                "No optimizer will be used; codebook may still update via EMA during forward."
            )
        optimizer = None
    else:
        optimizer = torch.optim.Adam(q_params, lr=cfg.lr)
...
# Training
    for epoch in range(cfg.epochs):
        ...
        model.train()  # IMPORTANT: keep train() so EMA updates in quantizer run
        if is_main_process():
        ...
        for images, _ in iterator:
            ...
            # -------------------------
            # OPTION A: EMA-only (no optimizer / no backward)
            # -------------------------
            ...
            # -------------------------
            # Normal training path (optimizer exists)
            # -------------------------
            else:
